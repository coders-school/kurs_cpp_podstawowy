...
Fruit::Fruit(size_t amount, const std::string& name, size_t base_price, size_t expiry_date, Time* time)
...
Fruit::Fruit(size_t amount, const std::string& name, size_t base_price, size_t expiry_date, size_t time_elapsed, Time* time)
    : Fruit(amount, name, base_price, expiry_date, time) {
    time_elapsed_ = time_elapsed;
}

Fruit::Fruit(size_t amnount, const Fruit* fruit, Time* time)
    : Fruit(amnount, fruit->GetName(), fruit->GetBasePrice(), fruit->GetExpiryDate(), fruit->GetTimeElapsed(), time) {
}
...
bool Fruit::operator==(const Cargo& cargo) const {
    ...
    return fruit.GetBasePrice() == base_price_ && fruit.GetName() == name_ && fruit.GetExpiryDate() == expiry_date_ && fruit.GetTimeElapsed() == time_elapsed_;
}
