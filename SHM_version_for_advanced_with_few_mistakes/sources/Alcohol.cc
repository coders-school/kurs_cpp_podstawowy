...
Alcohol::Alcohol(size_t amnount, const Alcohol* alcohol)
    : Alcohol(amnount, alcohol->GetName(), alcohol->GetBasePrice(), alcohol->GetPercentage()) {
}
...
bool Alcohol::operator==(const Cargo& cargo) const {
    ...
    return alcohol.GetBasePrice() == base_price_ && alcohol.GetName() == name_ && alcohol.GetPercentage() == percentage_;
}