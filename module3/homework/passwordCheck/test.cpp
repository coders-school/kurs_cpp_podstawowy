...
TEST(getErrorMessageTests, returnsErrorCodeAsString) {
    ...
    EXPECT_EQ(getErrorMessage(ErrorCode::PasswordNeedsAtLeastNineCharacters),
       "Too short password(needs at least 9 character )");
    ...
    EXPECT_EQ(getErrorMessage(ErrorCode::PasswordsDoesNotMatch), "Passwords doesn't match");
    EXPECT_EQ(getErrorMessage(static_cast<ErrorCode>(404)), "undefined error");
}
