...
TEST(getErrorMessageTests, returnsErrorCodeAsString) {
    ...
    EXPECT_EQ(getErrorMessage(ErrorCode::PasswordNeedsAtLeastOneSpecialCharacter), "Password neads at leastone special character.");
    EXPECT_EQ(getErrorMessage(ErrorCode::PasswordNeedsAtLeastOneUppercaseLetter), "Passwors needs at least one uppercase letter.");
